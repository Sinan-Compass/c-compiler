...
void toAssembly::generateArithmetic() {
	...
	if (q[0] == "+") {
		// ?????????
		assembly_code.push_back(format("MOV {}, {}", "AX", q[1])); // ???????AX?????
		...
		// ?????????
		assembly_code.push_back(format("MOV AX, {} ", q[1]));	//?????????????????AX?????
		assembly_code.push_back(format("IMUL {} ", q[2]));		// ??????????????

		assembly_code.push_back(format("LEA BX, {} ", q[3]));	//??????????
		assembly_code.push_back(format("MOV DW PTR [BX], AX ")); //??AX??????¦Ë
		assembly_code.push_back(format("ADD BX, 2 ", q[3]));	//???+2
		assembly_code.push_back(format("MOV DW PTR [BX], DX ")); //??DX??????¦Ë
	}
	else if (q[0] == "/") {
		// ??????????
		//????????????¦Ë????¦Ë????AX??DX?????
...
void toAssembly::generateLogic() {
	...
	if (q[0] == ">") {
		...
		assembly_code.push_back(format("JG {}", lable_right)); // ????????q[3]???AX > q[2]
		...
		assembly_code.push_back(format("MOV {}, {}", "AX", q[1])); // ???????AX?????
		assembly_code.push_back(format("CMP AX, {}", q[2]));
		assembly_code.push_back(format("JGE {}", lable_right)); // ????????q[3]???AX > q[2]
		...
		assembly_code.push_back(format("MOV {}, {}", "AX", q[1])); // ???????AX?????
		assembly_code.push_back(format("CMP AX, {}", q[2]));
		assembly_code.push_back(format("JL {}", lable_right)); // ????????q[3]???AX > q[2]
		...
		assembly_code.push_back(format("MOV {}, {}", "AX", q[1])); // ???????AX?????
		assembly_code.push_back(format("CMP AX, {}", q[2]));
		assembly_code.push_back(format("JLE {}", lable_right)); // ????????q[3]???AX > q[2]
		...
		assembly_code.push_back(format("MOV {}, {}", "AX", q[1])); // ???????AX?????
		assembly_code.push_back(format("CMP AX, {}", q[2]));
		assembly_code.push_back(format("JE {}", lable_right)); // ????????q[3]???AX > q[2]
		...
		assembly_code.push_back(format("MOV {}, {}", "AX", q[1])); // ???????AX?????
		assembly_code.push_back(format("CMP AX, {}", q[2]));
		assembly_code.push_back(format("JNE {}", lable_right)); // ????????q[3]???AX > q[2]
		...
		assembly_code.push_back(format("JE {}", lable_one_right)); // ???AX == 0??????????????
		assembly_code.push_back(format("JMP {}", lable_wrong));

		//??????§Ø????
		assembly_code.push_back(format("{}:", lable_one_right));
		assembly_code.push_back(format("MOV {}, {}", "AX", q[2])); // ?????????????????
		assembly_code.push_back(format("CMP AX, 1"));
		assembly_code.push_back(format("JE {}", lable_two_right)); // ???AX == 0??????????????
		assembly_code.push_back(format("JMP {}", lable_wrong));
		...
		assembly_code.push_back(format("MOV {}, {}", "AX", q[1])); // ???????AX?????
		assembly_code.push_back(format("CMP AX, 1"));
		assembly_code.push_back(format("JE {}", lable_right)); // ???AX == 0??????????????
		...
		assembly_code.push_back(format("JNE {}", lable_1)); // ???AX == 0??????????????
...
void toAssembly::generateDefFunction() {
	...
	string returnType = quats[cur_ptr][2]; // ????????????
